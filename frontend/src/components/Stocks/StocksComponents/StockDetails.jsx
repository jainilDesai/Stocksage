...
function StockDetails({ symbol2, title, w }) {
  ...
  const baseUrl = `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=${symbol}&apikey=${
    symbol === "IBM" ? "demo" : apiKey
  }`;
  ...
  const getStockInfo = (symbol) => {
    const selectedStock = stocksArr.find((stock) => {
      return stock.Symbol === symbol;
    });
  ...
  const getBoughtStocks = () => {
    ...
    if (boughtStocks) {
      totalBoughtStocks = boughtStocks.reduce(function (acc, investment) {
        return acc + Number(investment.Quantity);
      }, 0);
    }
    let totalSoldStocks = 0;
    if (soldStocks) {
      totalSoldStocks = soldStocks.reduce(function (acc, investment) {
        return acc + Number(investment.Quantity);
      }, 0);
    }
            ...
            >
              <Box display="flex" alignItems="center" gap="10px">
                ...
                <Box>
                  ...
                  >
                    <Heading>
                      <strong>
                        {stockInfo.CompanyName.length > 20
                          ? stockInfo.CompanyName.slice(0, 20) + "..."
                    ...
                    <Button
                      ...
                      onClick={
                        hasStock(symbol)
                          ? () => handleDeleteFromWatchList(stockInfo)
                          : () => handleAddToWatchlist(stockInfo)
                      }
                    >
                      {hasStock(symbol) ? (
              ...
              <Box display="flex" flexDirection="column" gap="5px">
                ...
                <Heading
                  ...
                  className={
                    stockInfo.PriceChange.includes("-")
                      ? "stock-down"
                      : "stock-up"
                  }
                  fontWeight="600"
              ...
              <DrawerContent p="20px">
                ...
                >
                  ...
                  <Button
                    ...
                    isDisabled={
                      selectedOption === "buy"
                        ? Number(stocksToBuy) === 0 ||
                          Number(stocksToBuy).toFixed(0) >
                            Math.floor(Number(balance) / Number(chartData.y[0]))
                        : Number(stocksToBuy) === 0 ||
                          Number(stocksToBuy) > stocksToSell
                    }
                    w="100%"
